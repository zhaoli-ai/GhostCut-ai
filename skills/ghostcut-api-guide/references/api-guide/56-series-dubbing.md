...
| 字幕输入 | 必须同时传 `workDto.idVeMaterialSrt` 和 `workDto.extraOptions.customer_input.content[]`；不能完全不传字幕，也不要只传其中一个。 |
...
| 结果查询 | 生产接入推荐通过 `callback` 接收结果；也可用 [任务查询](./53-series-edit-task-list.md) 定位任务。需要作品 ID、播放地址或详情时，再用任务 ID 调用 [视频任务状态查询](./11-work-status-query.md)。 |

## 推荐使用流程

1. 先通过 [项目与视频素材](./60-series-project-and-video-materials.md) 准备 `idSeries` 和 `idMaterialVideo`。
2. 准备字幕素材和逐句内容：
   - 先通过 [字幕素材管理](./61-series-subtitle-materials.md) 上传、创建、查询、提取或翻译字幕，拿到 `workDto.idVeMaterialSrt`。
   - 再从字幕素材的 `slInfo.sl[]`、业务侧人工编辑结果或用户提交的逐句台词中组装 `workDto.extraOptions.customer_input.content[]`。
   - `/series/edit/task/dubbing` 不允许完全不传字幕；经典模式和情感克隆模式都必须同时提供 `idVeMaterialSrt` 和 `customer_input.content[]`。
   - 如果需要先把字幕翻译成目标语种，先执行 [字幕翻译任务](./62-series-subtitle-translation.md)。
3. 先选择配音模式：
...
6. 调用本文接口提交配音任务。
7. 提交后生产接入推荐通过 `callback` 接收结果；也可调用 [任务查询](./53-series-edit-task-list.md) 判断任务是否完成。如果还需要作品详情、作品 ID 或播放地址，再按 [视频任务状态查询](./11-work-status-query.md) 调用 `/v-w-c/gateway/ve/work/status`。

## 字幕输入来源

AI 配音至少需要一份可用的字幕素材和对应的逐句内容。`/series/edit/task/dubbing` 不支持在完全不传字幕的情况下由系统默认 ASR 提取字幕；经典模式和情感克隆模式都必须同时提供下列两个字段：

| 字段 | 位置 | 作用 |
| --- | --- | --- |
| 字幕素材 ID | `workDto.idVeMaterialSrt` | 绑定项目、视频和字幕素材上下文。 |
| 逐句字幕内容 | `workDto.extraOptions.customer_input.content[]` | 明确本次配音使用的时间轴、角色、原文、译文和单句发音策略。 |

### `idVeMaterialSrt` 从哪里获取

`workDto.idVeMaterialSrt` 是字幕素材 ID，常见来源如下：

- 本地 SRT 上传：走 [文件上传](./10-file-upload.md) 的 `materialFileType=srt_series` 流程，上传凭证响应中的 `body.idMaterialSrt` 就是字幕素材 ID；后续仍建议通过 `series/srt/list` 再确认一次。
- 创建字幕素材：调用 [字幕素材管理](./61-series-subtitle-materials.md) 的 `/v-w-c/gateway/ve/series/srt/create`，响应 `body.id` 可作为 `idVeMaterialSrt`。
- ASR/OCR 字幕提取：字幕提取任务成功后，通过 `series/srt/list` 查询对应 `idSeries`、`idVeMaterialVideo`、`sourceLang` 和 `subtitleFrom`，用返回对象的 `body[].id`。
- 字幕翻译：翻译任务成功后，通过 `series/srt/list` 查询 `subtitleFrom=4` 的译后字幕，用返回对象的 `body[].id`。
- 字幕复制或已有字幕列表：通过 `series/srt/list` 精确查询，使用返回对象的 `body[].id`。

`idMaterialSrt` 和 `idVeMaterialSrt` 表示同一个字幕素材 ID；剪辑任务里字段名统一写 `workDto.idVeMaterialSrt`。

### `customer_input.content[]` 从哪里组装

- 需要手工指定每句台词的开始时间、结束时间、角色、原文和译文。
- 需要把人工修订后的台词直接作为本次配音的输入。
- 需要为部分句子补充角色、发音策略或人工修订标记。
- 可以从字幕列表返回的 `slInfo.sl[]` 解析，也可以使用业务侧人工编辑后的台词数组。
...
`workDto.idVeMaterialSrt` 和 `workDto.extraOptions.customer_input.content[]` 在译制出海 AI 配音中是配套关系：

- `idVeMaterialSrt` 提供字幕素材 ID 和项目上下文。
- `customer_input.content[]` 提供本次实际使用的逐句内容。
- 不要只传 `idVeMaterialSrt`，也不要只传 `customer_input.content[]`。
...
- 仍然必须同时提供 `workDto.idVeMaterialSrt` 和 `workDto.extraOptions.customer_input.content[]`。不能完全不传字幕，也不要只传其中一个。
...
剧集角色通过 [译制出海剧集角色管理](./65-series-character-management.md) 查询。角色列表返回对象的 `id` 对应 `character_voices[].id_ve_character`。`id_ve_character` 标识当前剧集中的业务角色，`id_ve_voice_character` 标识用于生成语音的公共音色；两者必须分别查询，不能互换。
...
- 剧集角色 ID 不应从公共音色结果读取；应以当前剧集角色列表或字幕 `slInfo.sl[].id_ve_character` 为准。
...
| `needChineseOcclude` / `videoInpaintLang` / `videoInpaintMasks` / `videoInpaintMasks_lite` / `videoInpaintMasks_pro` / `vtrMode` / `vtrProModel` | 去字幕相关字段，不属于 AI 配音最小请求体。是否需要先做字幕擦除，取决于原素材是否已无硬字幕，以及本次配音是否已通过 `workDto.materialWorkIds` 复用字幕擦除后的作品；如果两者都不是，应询问用户是否需要先按 [译制出海字幕擦除](./55-series-subtitle-inpaint.md) 创建前序任务，再用 `materialWorkIds` 复用擦除后的作品。 |
...
译制出海 AI 配音中的新字幕样式，沿用 [字幕样式和字体配置补充](./26-subtitle-style-and-fonts.md) 的规则。本文只保留组装配音任务时必须知道的部分，完整的 V1/V2 字段、字体文件、字号缩放和背景配置请看该补充文档。
...
适合需要精细控制字体、描边、阴影、背景和多语种字体文件的场景。配音任务中可在 `font_param` 中补充 `subtitleLang`、`videoWidth`、`fontFileName` 和 `config`；完整字段含义以 [字幕样式和字体配置补充](./26-subtitle-style-and-fonts.md) 为准。
...
- 正确流程是：先查 [任务查询](./53-series-edit-task-list.md) 取得前一步任务 ID，再按 [视频任务状态查询](./11-work-status-query.md) 调用 `/work/status`，从返回的 `body.content[].id` 读取可复用的作品 ID。

## 结果查询与作品获取

提交配音任务后，不要只看提交接口是否返回成功。生产接入推荐通过 `callback` 接收最终结果；需要主动查询或补偿兜底时，按下面的顺序查询：

1. 调用 [任务查询](./53-series-edit-task-list.md)。
...
- 先通过字幕列表确认 `workDto.idVeMaterialSrt` 属于同一 `idSeries` 和对应视频素材。
- 经典模式需要手动配置音色时，角色可以从字幕素材的 `slInfo.sl[].character` 和 `slInfo.sl[].id_ve_character` 提取；如果业务已接入角色列表接口，也可以从该接口获取本部剧中的全部角色后再选择需要配音的角色。
- 角色列表对象的 `id` 映射为 `id_ve_character`；公共音色列表对象的 `id` 映射为 `id_ve_voice_character`。不要因为两个接口都返回 `id` 就混用。
- `workDto.idVeMaterialSrt` 和 `workDto.extraOptions.customer_input.content[]` 必须同时传；不要只传其中一个。
- 如果配音前需要翻译字幕，先使用字幕翻译任务生成目标语种字幕，再把译后字幕用于配音。

## 配音专属检查清单

- `idSeries` 和 `idMaterialVideo` 必须属于同一个项目上下文；`idVeMaterialSrt` 也必须属于同一项目上下文。
- 是否同时传了 `idVeMaterialSrt` 和 `customer_input.content[]`；不要只传其中一个。
...
- [通用任务结构](./52-series-edit-common-task-structure.md)：查看 `customer_input`、`wyVoiceParam` 结构和 Python 提交通用模板。
- [视频任务状态查询](./11-work-status-query.md)：通过任务 ID 继续查询作品详情和播放地址。
- [公共音色查询接口](./32-public-voice-characters.md)：获取音色 ID 和可用语种。
- [译制出海剧集角色管理](./65-series-character-management.md)：查询或维护剧集角色，并获取 `id_ve_character`。
...
- [字幕素材管理](./61-series-subtitle-materials.md)：获取 `idVeMaterialSrt` 或解析字幕内容。
- [字幕翻译任务](./62-series-subtitle-translation.md)：先翻译字幕再配音。
- [翻译术语库](./63-series-translation-glossary.md)：固定角色名和专有名词译法。
- [错误与检查清单](./59-series-edit-errors-and-checklist.md)：排查音色与语种不匹配等问题。

## Agent 决策规则

- 用户要“译制出海配音”“AI 配音”“翻译并配音”时，使用本接口。
- 经典模式必须使用 `wyTaskType=FULL`，并通过 `character_voices[]` 手动选择角色音色。
- 情感克隆模式传 `wyTaskType=VOICE_CLONE_PRO`，不需要传 `character_voices[]`，但仍然必须同时提供 `idVeMaterialSrt` 和 `customer_input.content[]` 作为字幕输入。
- 提交后生产接入推荐通过 `callback` 接收结果；需要主动查询或补偿兜底时先查 `task/list`，需要作品详情或播放地址时再查 `/work/status`。
...
- 译制出海配音必须同时使用 `idVeMaterialSrt` 和 `customer_input.content[]`：前者绑定字幕素材上下文，后者明确本次实际配音的时间轴、角色和文本。
...
- 剧集角色 ID 通过 [译制出海剧集角色管理](./65-series-character-management.md) 或字幕 `slInfo.sl[]` 获取；不要用公共音色 ID 代替。
...
- 判断是否需要先做字幕擦除：如果原素材已确认无硬字幕，直接创建配音任务；如果已经通过 `workDto.materialWorkIds` 复用字幕擦除后的作品，不要再次创建字幕擦除任务；如果原素材可能有硬字幕且未复用擦除后作品，应询问用户是否需要先擦除原字幕。
