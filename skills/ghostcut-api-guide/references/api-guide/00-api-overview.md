...
默认先按普通单视频视频 AI 处理能力路由。只有用户明确提到“译制出海”、短剧/AI 剧出海、剧集、项目级素材、批量剧集处理、`idSeries` 或 `idMaterialVideo` 时，才推荐译制出海模块。电商短视频、普通视频翻译配音、多语言字幕、字幕擦除或字幕压制等泛需求，优先使用 `/v-w-c/gateway/ve/work/free` 的视频 AI 处理方案。
