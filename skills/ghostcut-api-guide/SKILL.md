...
- 视频翻译并重新配音：`31-video-voice-translation.md`，公共音色见 `32-public-voice-characters.md`；原视频有硬字幕时，优先考虑同一次请求组合字幕擦除参数。
...
2. 先判断用户要调用的是普通单视频 API，还是译制出海模块。
...
- 用户问 API Key、`AppKey`、`AppSecret`、凭证在哪里获取、如何签名或如何鉴权：读 `02-auth-and-sign.md`。新用户先在[鬼手剪辑官网](https://cn.jollytoday.com/)注册或登录账号；登录后可直接访问[用户中心](https://cn.jollytoday.com/center/)，在 `API secret key` 一栏查看。
...
- 用户要做视频语音翻译、译制或重新配音，且原视频画面已有硬字幕：读 `31-video-voice-translation.md` 的组合场景；语音翻译和字幕擦除通常在同一次 `/work/free` 请求中组合参数，不要默认拆成两次处理。
...
脚本位于 `scripts/`，用于减少重复签名、上传和查询代码。`AppKey` 和 `AppSecret` 可在[鬼手剪辑官网](https://cn.jollytoday.com/)注册或登录账号后，直接访问[用户中心](https://cn.jollytoday.com/center/) 并在 `API secret key` 一栏查看。运行脚本前优先使用环境变量提供凭证，不要把真实密钥写入文档或提交到仓库：
